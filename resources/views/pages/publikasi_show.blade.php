        ...
        <div class="lg:col-span-8 xl:col-span-9 flex flex-col gap-6">
            ...
            <article class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div class="p-6 sm:p-10 lg:p-12">
                    ...
                    <!-- Bagikan Artikel -->
                    <div
                        class="bg-indigo-50/50 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 border border-indigo-100/50 mt-12 mb-2">
                        <div>
                            <h4 class="text-indigo-950 font-bold mb-1 text-sm uppercase tracking-wider">Bagikan Publikasi
                                Ini
                            </h4>
                            <p class="text-xs font-semibold text-slate-500">Sebarkan informasi ini ke jejaring sosial Anda.
                            </p>
                        </div>
                        <div class="flex items-center gap-3">
                            <!-- WhatsApp -->
                            <a href="https://api.whatsapp.com/send?text={{ urlencode($publication->title . ' - ' . request()->url()) }}"
                                target="_blank"
                                class="w-10 h-10 rounded-xl bg-white border border-[#25D366]/20 text-[#25D366] hover:bg-[#25D366] hover:text-white flex items-center justify-center transition-all shadow-sm hover:shadow-[#25D366]/30 group"
                                title="Bagikan ke WhatsApp">
                                <svg class="w-5 h-5 group-hover:scale-110 transition-transform" fill="currentColor"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M12.031 21.488a10.021 10.021 0 01-5.111-1.39l-5.69 1.492 1.517-5.545a10.016 10.016 0 119.284 5.443z"
                                        stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                        stroke-linejoin="round" fill="none" />
                                    <path
                                        d="M16 14.5c-.26-.13-1.54-.76-1.78-.85-.24-.09-.42-.13-.6.13-.18.26-.68.85-.83 1.02-.15.17-.31.19-.57.06-.26-.13-1.1-.41-2.1-1.3-3.1-2.73-3.41-3.26-3.41-3.26s-.04-.08 0-.16c.03-.06.13-.15.19-.23.06-.08.08-.13.13-.22.04-.09.02-.17-.02-.23-.04-.06-.6-1.44-.82-1.97-.21-.52-.43-.45-.6-.46-.15-.01-.33-.01-.51-.01-.18 0-.48.06-.72.33-.24.26-.92.9-.92 2.19s.94 2.54 1.07 2.72c.13.18 1.86 2.84 4.5 3.98.63.27 1.12.43 1.5.55.63.2 1.2.17 1.65.1.51-.08 1.54-.63 1.76-1.24.22-.61.22-1.13.15-1.24-.06-.09-.23-.15-.49-.28z" />
                                </svg>
                            </a>
                            <!-- FB -->
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->url()) }}"
                                target="_blank"
                                class="w-10 h-10 rounded-xl bg-white border border-[#1877F2]/20 text-[#1877F2] hover:bg-[#1877F2] hover:text-white flex items-center justify-center transition-all shadow-sm hover:shadow-[#1877F2]/30 group"
                            ...
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($publication->title) }}"
                                target="_blank"
                                class="w-10 h-10 rounded-xl bg-white border border-slate-800/20 text-slate-800 hover:bg-slate-800 hover:text-white flex items-center justify-center transition-all shadow-sm hover:shadow-slate-800/30 group"

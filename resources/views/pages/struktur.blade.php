...
@section('content')
    <div class="bg-slate-900 pb-32 pt-16 relative overflow-hidden">
        <div class="absolute inset-0 opacity-[0.05] text-white">
            <svg class="h-full w-full" xmlns="http://www.w3.org/2000/svg">
                <defs>
                    <pattern id="pattern-struktur" x="0" y="0" width="60" height="60" patternUnits="userSpaceOnUse">
                ...
                <rect x="0" y="0" width="100%" height="100%" fill="url(#pattern-struktur)"></rect>
            </svg>
        </div>

        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            <h1 class="text-3xl md:text-5xl font-extrabold text-white brand-font tracking-tight mb-4">Struktur Organisasi</h1>
    ...
    <div class='bg-white rounded-3xl shadow-xl shadow-slate-200/50 p-8 sm:p-12 border border-slate-100'>
        ...
        <div class='prose prose-slate max-w-none prose-headings:brand-font prose-headings:text-slate-800'>
            @if(isset($settings['page_struktur']) && !empty($settings['page_struktur']))
                {!! $settings['page_struktur'] !!}
            @else
                <p class='lead text-lg text-slate-600 mb-6'>Bagian ini berisi informasi mengenai Struktur Organisasi.</p>
